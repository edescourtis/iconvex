defmodule Iconvex.Specs.ICUArchiveShardB.Application do
  @moduledoc false
  use Application

  @ids for index <- 351..700, do: String.to_atom("icu_archive_#{index}")
  @impl true
  ...
  defp register_providers do
    Enum.reduce_while(@ids, {:ok, []}, fn id, {:ok, owned} ->
      case Iconvex.Tables.register_provider_owned(id, :iconvex_specs_icu_archive_b) do
        {:ok, :existing} ->
          {:cont, {:ok, owned}}
  ...
  defp unregister_providers(providers) do
    Enum.each(providers, fn {id, token} ->
      Iconvex.Tables.unregister_provider(id, :iconvex_specs_icu_archive_b, token)
    end)
  end
end
