defmodule Iconvex.Specs.VSCII2Benchmark.Reference do
  @moduledoc false
...
defmodule Iconvex.Specs.VSCII2Benchmark do
  alias Iconvex.Specs.VSCII2
  alias Iconvex.Specs.VSCII2.SourceAsset
  alias Iconvex.Specs.VSCII2Benchmark.Reference
  ...
  def run do
    IO.puts("schema\ticonvex-vscii2-benchmark\t1")
    ...
    for corpus_name <- [:ascii, :combining, :extended, :complete_defined] do
      ...
      benchmark(
        corpus_name,
        "decode_to_utf8",
        {fn -> VSCII2.decode_to_utf8(timing.source) end, {:ok, timing.text}},
        {fn -> VSCII2.decode_to_utf8(small.source) end, {:ok, small.text}},
        {fn -> VSCII2.decode_to_utf8(large.source) end, {:ok, large.text}},
      ...
      benchmark(
        corpus_name,
        "encode_from_utf8",
        {fn -> VSCII2.encode_from_utf8(timing.text) end, {:ok, timing.source}},
        {fn -> VSCII2.encode_from_utf8(small.text) end, {:ok, small.source}},
        {fn -> VSCII2.encode_from_utf8(large.text) end, {:ok, large.source}},
    ...
    IO.puts(
      "comparator\tgnu-libiconv-1.19\tunavailable\t" <>
        "VSCII-2/VN2/ISO-IR-180 are absent from default and " <>
        "--enable-extra-encodings catalogs; VISCII and TCVN/VN1 are distinct codecs"
    )

    IO.puts("summary\t1 exact VN2 profile\t4 corpora\tall performance gates passed")
  end
  ...
  defp gate!(corpus, operation, scaling, relative, rate) do
    label = "VSCII-2 #{corpus} #{operation}"
  ...
  defp corpus(:combining, units), do: build_corpus(<<0xB0, 0xB1, 0xB2, 0xB3, 0xB4>>, units)

  defp corpus(:extended, units),
    do: build_corpus(:erlang.list_to_binary(Enum.to_list(0xA0..0xFF)), units)

  defp corpus(:complete_defined, units) do
    alphabet = :erlang.list_to_binary(Enum.to_list(0x00..0x7F) ++ Enum.to_list(0xA0..0xFF))
    build_corpus(alphabet, units)
  end

  defp build_corpus(alphabet, units) do
    source = repeat_to_size(alphabet, units)
    {:ok, text} = VSCII2.decode_to_utf8(source)
    {:ok, ^source} = VSCII2.encode_from_utf8(text)
...
Iconvex.Specs.VSCII2Benchmark.run()
